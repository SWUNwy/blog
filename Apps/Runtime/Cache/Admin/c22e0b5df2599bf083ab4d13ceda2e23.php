        ...
        <div class="mt-20">
            <table class="table table-border table-bordered table-hover table-bg table-sort">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                        <td><input name="id[]" id="checkbox" type="checkbox" value="<?php echo ($vo["category_id"]); ?>"></td>
                        <td><?php echo ($vo["sort_id"]); ?></td>
                        <td><?php echo ($vo["category_id"]); ?></td>
                        <td class="text-c"><?php echo ($vo["name"]); ?></td>
                        <td><?php if($vo["parent_id"] == 0): ?>一级分类<?php else: echo ($vo["parent_id"]); endif; ?></td>
                    <td><?php if($vo['top'] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
                    <td class="f-14">
                        <a title="编辑" href="<?php echo U(" Category/editCategory?id=$vo[category_id]");?>" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
                        <a title="删除" href="<?php echo U(" Category/delete?id=$vo[category_id]");?>" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                    </td>
